...
Page({
    ...
    filterProvince(e) {
        ...
        if (!keyword) {
            this.setData({
                DisplayProvince: listdefault
            })
            return
        }

        const result = listdefault.filter(item => {
            const en = (item.province_name_en || "").trim().toLowerCase().replace(/\s+/g, "")
            const cn = (item.province_name || "").trim()
            const combined = (en + cn).toLowerCase()
